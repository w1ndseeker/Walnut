...
Image::Image(std::string_view path) : m_Filepath(path) {
    ...
    if (stbi_is_hdr(m_Filepath.c_str())) {
        data = (uint8_t *)stbi_loadf(m_Filepath.c_str(), &width, &height,
                                     &channels, 4);
...
void Image::SetData(const void *data) {
    ...
    if (!m_StagingBuffer) {
        // Create the Upload Buffer
        {
            ...
            err =
                vkCreateBuffer(device, &buffer_info, nullptr, &m_StagingBuffer);
            check_vk_result(err);
            VkMemoryRequirements req;
            ...
            check_vk_result(err);
            err = vkBindBufferMemory(device, m_StagingBuffer,
    ...
    // Copy to Image
    {
        ...
        vkCmdCopyBufferToImage(command_buffer, m_StagingBuffer, m_Image,
